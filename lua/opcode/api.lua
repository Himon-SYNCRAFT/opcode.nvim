	...
	vim.system(cmd, {}, function(obj)
		if obj.code ~= 0 then
			return vim.schedule(function()
				callback({ message = "Network error", code = obj.code, stderr = obj.stderr })
			end)
		end
		...
		if not status or status < 200 or status >= 300 then
			return vim.schedule(function()
				callback({ message = "HTTP error", status = status, body = body_str })
			end)
		end
		...
		if not ok then
			return vim.schedule(function()
				callback({ message = "JSON parse error", details = data })
			end)
		end
